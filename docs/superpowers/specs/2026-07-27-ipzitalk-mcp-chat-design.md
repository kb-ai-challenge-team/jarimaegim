...
> **정정 (2026-07-27, 구현 후)** — 이 문서의 도구 표는 처음에 ipzitalk README의 소개 표를 보고
> 작성됐다. 실제 `presale-mcp` 서버에 `tools/list`를 질의해 대조한 결과, 원시 도구 11종 중
> **3종은 존재하지 않았고 4종은 인자 이름이 달랐다.** 특히 `get_region_code`가 `address`가
> 아니라 `query`를 요구해, 다른 모든 도구의 선행 단계인 `resolve_seoul_place`가 실제 키를
> 넣는 순간 실패하는 상태였다. 테스트 191개가 전부 통과했지만 전부 가짜 세션을 쓴 탓에
> 아무것도 잡지 못했다.
>
> **README는 스키마가 아니다.** 아래 표는 서버 스키마 기준으로 정정됐고, 도구는 7종에서
> 6종으로 줄었다(`render_location_map`은 받칠 원시 기능이 없어 삭제).
