...
## 외부 계약 규율 — 문서가 아니라 시스템에 묻는다

이 계획을 실행하면서 가장 비싼 실수가 나왔다. `chat_tools.py`의 원시 도구 이름 11개와
인자 키를 전부 **ipzitalk README의 소개 표**를 보고 썼다. 그건 스키마가 아니라 마케팅용
설명이었다.

실제 `presale-mcp` 서버에 `tools/list`를 질의해 대조하니 **3종은 존재하지 않았고 4종은
인자 이름이 달랐다.** `get_region_code`가 `address` 대신 `query`를 요구하는 탓에, 다른
모든 도구의 선행 단계인 `resolve_seoul_place`가 실제 키를 넣는 순간 실패하는 상태였다.

**테스트 191개가 전부 통과하고 있었다.** 전부 가짜 세션을 쓰기 때문이다. 내가 상상한
서버를 상대로 완벽하게 검증한 셈이었다.

그러므로 외부 시스템의 계약을 코드에 박기 전에:

1. **문서가 아니라 시스템에 묻는다.** MCP 서버는 `tools/list`, HTTP API는 OpenAPI 스펙,
   라이브러리는 설치된 패키지의 소스. README·블로그·기억은 출처가 아니다.
2. **묻는 비용을 먼저 확인한다.** 이 경우 자격증명 없이 핸드셰이크만으로 전체 스키마를
   4초 만에 얻을 수 있었다. 몰라서 안 한 게 아니라, 물어볼 수 있다는 걸 몰랐다.
3. **가짜로 테스트하는 계층은 계약 검증을 못 한다.** 페이크는 우리 로직을 검증하지,
   상대가 그 모양인지는 검증하지 않는다. 이름·필수 인자만이라도 실물에서 뽑아 고정한다.

같은 규율이 이 계획의 다른 곳에서도 두 번 값을 했다. OpenAI Responses API 어댑터의 참조
코드는 Chat Completions 형식을 넘기고 있었고(구현자가 설치된 SDK 소스를 읽고 잡음),
하트비트 래퍼는 anyio 취소 스코프 규칙을 위반했다(구현자가 실제 서브프로세스로 재현).
