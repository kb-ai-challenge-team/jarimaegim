...
- **구를 펼쳤을 때 마커 15개가 전부 겹친다.** 계획 단계에서는 15건으로 줄이면 겹침을 피할 수 있다고 봤으나, 실측 결과 그 가정이 틀렸다. 1440×900 뷰포트에서 강남구를 펼치면 마커 15개가 지도 패널(558×900px) 안의 211×187px 영역에 몰리고 15개 전부가 다른 마커와 겹친다. 포커스된 하나의 라벨만 읽히고 나머지는 가려진다.
  - 다음 단계로 유력한 것은 클러스터링이 아니라 **비포커스 마커를 순번·등급만 있는 작은 핀으로 축약하고 포커스될 때만 전체 라벨을 펼치는 것**이다. 구현이 가볍고 겹침의 원인(라벨 폭)을 직접 줄인다.
- 클러스터링은 여전히 범위 밖이다.
