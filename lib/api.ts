...
import type { AnalysisResult, Candidate, CaseInput, CaseRecord, CostPlan, DocumentRecord, IntegrationStatus, Program } from "./types";
...
async function authorizedHeaders(init?: RequestInit) {
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  const response = await fetch(`${API_BASE}${path}`, { credentials: "include", ...init, headers: await authorizedHeaders(init) });
  if (!response.ok) throw await responseError(response);
  return await response.json() as T;
}

export const api = {
  status: () => request<{ integrations: IntegrationStatus; mode: string }>("/status"),
  ...
  downloadDocument: async (documentId: string) => { const response = await fetch(`${API_BASE}/documents/${documentId}/download`, { credentials: "include", headers: await authorizedHeaders() }); if (!response.ok) throw await responseError(response); return response.blob(); },
